...
{
  // Здесь постоянно запускаемся в цикле
  // Serial.println("WebServer::handleClient - подключаемся");
  ...
  {
    //Serial.println("Подключился текущий клиент");
    switch (_currentStatus) 
    {
      case HC_NONE:
        // No-op, чтобы избежать предупреждения компилятора C++
        break;
      case HC_WAIT_READ:
        // Ждем, пока данные от клиента станут доступны
        if (_currentClient.available()) 
        {
          Serial.println("Данные текущего клиента доступны");
