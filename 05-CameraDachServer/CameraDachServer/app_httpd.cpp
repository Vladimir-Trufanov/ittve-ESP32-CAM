...
// ****************************************************************************
// *                      Выбрать значение регистра OV2640                    *
// ****************************************************************************
/* 
Метод get_reg(s, reg, mask) у структуры sensor_t — это функция, которая позволяет 
читать значение регистра датчика, например, в контексте ESP32-камеры. 
Метод реализован в драйвере датчика, например, в файле ov2640.c, где:
  s — указатель на структуру управления датчиком, которую можно получить с помощью 
функции esp_camera_sensor_get();
  reg — адрес регистра;
...
// ****************************************************************************
// *     По запросу сервера передать один раз параметры потока изображений    *
// *          на экране управления в начале сессии главной страницы           *
// ****************************************************************************
static esp_err_t status_handler(httpd_req_t *req) 
{
  // Резервируем json-строку с параметрами потока изображений
  static char json_response[1024];
  // Выбираем ссылку на параметры камеры
  sensor_t *s = esp_camera_sensor_get();
